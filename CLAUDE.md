...
- `lib/ai/index.ts`: Anthropic client + `modelFor(portfolio, kind)` routing. Two tiers by whether the pass is **user-facing judgment** or invisible bookkeeping: `MODELS.reasoning` = `claude-opus-4-8` runs advisory/vision **plus** the conversational path — intent `route`, the spoken reply, and capture `classify` — for Claude-quality chat + fewer mis-routes; `MODELS.fast` = Haiku runs only the non-user-visible memory passes (`summarize`/`extract`). Office reasoning stays on the trusted first-party API. (This deliberately trades the earlier Haiku-first minimal-budget stance for chat quality — owner-approved.)
...
Every message hits `POST /api/orchestrator`: `buildContext` (bounded memory) → `routeIntent` (Opus online / deterministic heuristic offline) → `act` (dispatch) → `composeReply` (Opus, **given the assembled context**) → persist turns + `extractFacts`. **Ten intents**: `task`, `calendar`, `completion`, `status`, `question` (consult, never files), `handoff` (confirm via Inbox), `reminder` (creates real reminder rules, quiet-hours-aware), `edit` (reschedule a matched task, audited), `delete` (soft-cancel = recoverable), `chitchat` (a closing/acknowledgement — "thanks", "nothing further" — warm no-op, files nothing; full-message-anchored regex so it never swallows a real request). `edit`/`delete`/`completion` share `matchOpenTask` (keyword fuzzy match; clarifies when unsure). `edit`/`delete` also recognise **routine template blocks** via `matchTemplateBlock` (Gym / Walk, Study…): with a time, `edit` reschedules the block for that day (schedule exception vacates the slot + a timed item at the new time); without one, it clarifies that a routine block isn't a task (FR4). `calendar` proposes a re-plan **only** on a real same-day timed clash (`overlapsExistingTimedTask`), else files silently. **Replies converse** (`reply.ts`, Opus + context): a warm, grounded chief-of-staff voice that confirms what it actually did and may add a short observation or a **self-contained** clarifying question/next-step (phrased so the manager can act in one reply — not a bare yes/no the single-turn router couldn't field). It never claims an action it didn't take. Confirmation scales with stakes. Rich engines (decisions, initiatives, advisory, reports, people/waiting, search) are built but reachable via their **screens/APIs**, not (yet) via chat — see `tests/UAT Test Cases/PersonalCOS_Test_Cases.md` §0 for the executability map.
...
**Deployed and live** on Vercel (`main`, auto-deploy) + Neon (migrated through `0002` — 5 memory tables + RLS + `pcos_app` grants verified). Test suite **~212 green**; `main` is the source of the production deployment.

Conversational increment (Phases 1–8) shipped: data layer (5 tables + column alters), orchestrator (10 intents), memory core (context/facts/summary), conversation-first UI (session home + merged composer, voice/image retained), plan negotiation (propose→agree→commit), session lifecycle (open/sweep crons), retention (FR47) + Memory view (FR48), cost guardrails (NFR-10 token cap + per-turn audit).

Post-increment fixes & additions (all with tests):
...
- **Nav icons + live badge counts** (`lib/nav/counts.ts`, mockup parity).
- **Conversation quality (Path A):** replies **confirm-and-stop** (no leading questions / no hallucinated actions) + a `chitchat` intent for closings — fixes short follow-up replies mis-routing (e.g. "Nothing further. Thanks." → chitchat, not a failed completion).
- **Routine-block rescheduling (FR4):** `edit`/`delete` recognise template blocks (Gym / Walk, Study…); "reschedule my Gym/Walk to 7pm" moves it via a schedule exception + a timed item; without a time it clarifies (`matchTemplateBlock`/`blockDurationMin`).
- **Session UX:** auto-scroll to newest message, inline "Updated plan" republish after a plan-changing action, avatar initials from `displayName` ("Anand Iyer" → "AI").
- UAT: `tests/UAT Test Cases/` holds the test pack, a §0 executability assessment, results, and a `tests/uat/` harness.

Open follow-ups: **Robinhood connector** needs `ROBINHOOD_MCP_URL` set in Vercel + a per-user token POSTed to `/api/connectors/robinhood` (no connect UI yet); FR48 fact **edit** in the UI; the reminder slot-filling **Path B** (guided multi-turn reminder flow); several 🖥️/❌ chat-wiring gaps (decisions/initiatives/reports/people/search) documented in the UAT §0 tally. Quiet-hours + retention-cron timing are **server-local/UTC** (retention scheduled `30 7 * * *` = 03:30 EDT). Note: unauthenticated requests resolve to the **dev-fallback tenant** (which in this prod config is the owner's own tenant), so avoid unauthenticated writes against prod.
